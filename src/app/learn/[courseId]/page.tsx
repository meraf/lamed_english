...
type PageProps = {
  // Folder is [courseId], so 'id' must be 'courseId'
...
export default async function LessonPage({ params, searchParams }: PageProps) {
  ...
  const courseUrlId = resolvedParams.courseId;
  const activeTab = resolvedSearchParams.tab || "video";

  const session = await getServerSession(authOptions);
  if (!session || !session.user?.email) redirect("/api/auth/signin");

  // METHOD 1: Find the first lesson associated with this course ID
  const lesson = await prisma.lesson.findFirst({
    where: { 
        courseId: courseUrlId 
    },
    orderBy: {
        order: 'asc' // Ensures we get the first lesson
    },
    include: {
      course: {
        include: {
  ...
  // If the course has no lessons yet
  if (!lesson) {
    return (
        <div className="flex flex-col items-center justify-center h-screen bg-slate-50">
            <p className="font-black uppercase text-slate-400">No lessons found for this course.</p>
            <Link href="/dashboard" className="mt-4 text-xs font-bold text-blue-600 underline">Return to Dashboard</Link>
        </div>
    );
  }

  // Define the ID for logic below
  const lessonUrlId = lesson.id;
  ...
  const currentIndex = allLessons.findIndex((l: any) => l.id === lessonUrlId);
      ...
      <aside className="w-full lg:w-80 bg-slate-50 border-r border-slate-100 flex flex-col h-full shrink-0">
        ...
        <nav className="p-4 space-y-2 flex-1 overflow-y-auto custom-scrollbar">
          {allLessons.map((l: any, idx: number) => (
            <Link key={l.id} href={`/learn/${courseUrlId}?lessonId=${l.id}&tab=${activeTab}`} className={`flex items-center justify-between p-3 rounded-xl transition-all ${l.id === lessonUrlId ? "bg-white shadow-sm border border-slate-200" : "hover:bg-slate-100"}`}>
        ...
        <div className="max-w-6xl mx-auto p-6 md:p-12">
          ...
          <div className="mb-12">
                    ...
                    <div key={exam.id} className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                      <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col h-[700px]">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Exam Paper</h3>
                            {exam.fileUrl && <a href={exam.fileUrl} target="_blank" className="text-blue-600 text-[10px] font-black uppercase flex items-center gap-1 hover:underline"><Download size={12}/> Download</a>}
                        </div>
                        <div className="flex-grow overflow-y-auto custom-scrollbar pr-2">
                          <p className="text-slate-700 text-sm whitespace-pre-wrap mb-6 font-medium leading-relaxed bg-slate-50 p-4 rounded-2xl border border-slate-100">{exam.description || "Instructions inside file."}</p>
                        </div>
                      </div>
                      ...
                      <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                        ...
                        {!result ? (
                          <form className="space-y-4" action="/api/exams/submit" method="POST" encType="multipart/form-data">
                            ...
                            <textarea name="textAnswer" rows={8} className="w-full p-4 rounded-2xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-slate-900 bg-slate-50/50" placeholder="Type answer here..."></textarea>
                            <input type="file" name="fileAnswer" className="text-xs w-full"/>
                          ...
                          <div className="h-full flex flex-col items-center justify-center text-center p-10 bg-green-50/50 rounded-3xl">
                            <CheckCircle2 size={56} className="text-green-500 mb-4" />
                            <p className="font-black uppercase text-lg text-slate-900">Exam Submitted</p>
                            <p className="text-slate-600 text-[10px] font-black uppercase">Score: <span className="text-green-600">{result.score ?? "Pending"}</span></p>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {activeTab === "assignments" && (
              <div className="space-y-10">
                ...
                {lesson.assignments.map((ass: any) => {
                      ...
                      <div className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                        {!submission ? (
                           <form action="/api/assignments/submit" method="POST" encType="multipart/form-data" className="space-y-6">
                            ...
                            <input type="file" name="submissionFile" required className="text-xs"/>
                            <button type="submit" className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest">Submit Work</button>
                          </form>
                        ) : (
                          <div className="text-center p-8 bg-blue-50/50 rounded-3xl">
                             <CheckCircle2 size={40} className="text-blue-500 mx-auto mb-4" />
                             <p className="font-black uppercase text-slate-900">Submitted</p>
                             <p className="text-blue-600/60 text-[10px] font-black uppercase">Status: {submission.status}</p>
                          </div>
                        )}
                      </div>
                    </div>
            ...
            {activeTab === "reading" && (
              <div className="bg-white p-10 md:p-16 rounded-[2.5rem] border border-slate-200 shadow-sm">
                ...
                <div className="prose prose-slate max-w-none text-slate-600 text-lg leading-relaxed mb-10 pb-10 border-b">
                  {lesson.content || "No notes for this lesson."}
                </div>
                ...
                {announcements.map((ann: any) => (
                  <div key={ann.id} className="bg-white p-8 rounded-[2rem] border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-2 mb-4">
                       ...
                       <span className="text-[10px] font-black text-slate-400 uppercase">{new Date(ann.createdAt).toLocaleDateString()}</span>
                    </div>
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 pt-8 border-t border-slate-100">
            <div>
              ...
              <p className="text-slate-400 font-bold uppercase text-[10px]">Lesson {currentIndex + 1} / {allLessons.length}</p>
            </div>
          ...
          <div className="flex justify-between pt-8 pb-20">
             {prevLesson ? (
               <Link href={`/learn/${courseUrlId}?lessonId=${prevLesson.id}&tab=${activeTab}`} className="font-black text-slate-400 hover:text-slate-900 text-[10px] tracking-widest uppercase flex items-center gap-2">
                 <ChevronLeft size={18}/> PREVIOUS
               </Link>
             ) : <div />}
             {nextLesson ? (
               <Link href={`/learn/${courseUrlId}?lessonId=${nextLesson.id}&tab=${activeTab}`} className="font-black text-slate-900 hover:text-blue-600 text-[10px] tracking-widest uppercase flex items-center gap-2">
